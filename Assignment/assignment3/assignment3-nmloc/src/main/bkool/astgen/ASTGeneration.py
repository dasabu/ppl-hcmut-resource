...
class ASTGeneration(BKOOLVisitor):
    ...
    def visitImmutDecl(self, ctx:BKOOLParser.ImmutDeclContext):
        ...
        constdecl_list = [ConstDecl(x[0], typ, x[1]) for x in self.visit(ctx.immutInitList())]
    ...
    def visitImmutInitList(self, ctx:BKOOLParser.ImmutInitListContext):
        if ctx.getChildCount() == 1:
            return self.visit(ctx.immutInit())
        else:
            return self.visit(ctx.immutInit()) + self.visit(ctx.immutInitList())
    
    # immutInit: idlist EQUAL_SIGN expr;
    def visitImmutInit(self, ctx:BKOOLParser.ImmutInitContext):
        # x, y = 1 -> [[Id(x), IntLit(1)], [Id(y), IntLit(1)]]
        idlist = self.visit(ctx.idlist()) # [Id(x), Id(y)]
        expr = self.visit(ctx.expr()) # IntLiteral(1)
    ...
    def visitMutDecl(self, ctx:BKOOLParser.MutDeclContext):
        ...
        typ = self.visit(ctx.typ())
        vardecl_list = [VarDecl(x[0], typ, x[1]) for x in self.visit(ctx.mutMemberList())]
    ...
    def visitMutMemberList(self, ctx:BKOOLParser.MutMemberListContext):
        res = [self.visit(x) for x in ctx.mutMember()]
    ...
    def visitImmutVardecl(self, ctx:BKOOLParser.ImmutVardeclContext):
        ...
        typ = self.visit(ctx.typ())
        immut_init_list = self.visit(ctx.immutVarBody())
        # x,y = 1, a = 1.2
    ...
    def visitImmutVarBody(self, ctx:BKOOLParser.ImmutVarBodyContext):
        if ctx.getChildCount() == 1:
            return self.visit(ctx.immutVarMem())
        else:
            return self.visit(ctx.immutVarMem()) + self.visit(ctx.immutVarBody())
    ...
    def visitMutVardecl(self, ctx:BKOOLParser.MutVardeclContext):
        typ = self.visit(ctx.typ())
        mut_decl_list = self.visit(ctx.mutVarBody())
