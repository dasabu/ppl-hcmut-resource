...
variable_decl: (mutable? STATIC? | STATIC mutable| ) attribute_decl SEMI;
mutable: FINAL;

attribute_decl:   attribute_list SEMI;
attribute_list: (not_initial | initial) COMMA attribute_list | (not_initial | initial);
not_initial: idlist;
initial: idlist EQ expr;

//---------Method declaration ---------
method_decl: STATIC type_name paramlist ;
...
//------- 6.1 assignment ------------
assignment_stmt: lhs EQ expr SEMI;
...
//------- 6.3 for in ------------------
for_stmt: FOR scalar_variable EQ expr (DOWNTO|TO) expr (stmt|block_stmt);
...
//----------4.2 array type----------
array_type: (INT | FLOAT | BOOLEAN | STRING) LS  (size)? RS;
size: INTLIT;
