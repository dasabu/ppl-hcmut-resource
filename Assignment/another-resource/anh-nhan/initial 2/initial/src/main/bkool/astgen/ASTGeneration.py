...
class ASTGeneration(BKOOLVisitor):
        ...
        if ctx.STATIC():
            if ctx.FINAL():
                for i in range(len(attribute_list)):
                    if type(type_name) == ClassType:
                        lst += [AttributeDecl(Static(), ConstDecl(attribute_list[i][0], type_name, NullLiteral()))]
                    else:
                        lst += [
                            AttributeDecl(Static(), ConstDecl(attribute_list[i][0], type_name, attribute_list[i][1]))]
            else:
                for i in range(len(attribute_list)):
                    if type(type_name) == ClassType:
                        lst += [AttributeDecl(Static(), VarDecl(attribute_list[i][0], type_name, NullLiteral()))]
                    else:
                        lst += [AttributeDecl(Static(), VarDecl(attribute_list[i][0], type_name, attribute_list[i][1]))]
        else:
            if ctx.FINAL():
                for i in range(len(attribute_list)):
                    if type(type_name) == ClassType:
                        lst += [AttributeDecl(Instance(), ConstDecl(attribute_list[i][0], type_name, NullLiteral()))]
                    else:
                        lst += [
                            AttributeDecl(Instance(), ConstDecl(attribute_list[i][0], type_name, attribute_list[i][1]))]
            else:
                for i in range(len(attribute_list)):
                    if type(type_name) == ClassType:
                        lst += [AttributeDecl(Instance(), VarDecl(attribute_list[i][0], type_name, NullLiteral()))]
                    else:
                        lst += [
                            AttributeDecl(Instance(), VarDecl(attribute_list[i][0], type_name, attribute_list[i][1]))]
        return lst
    ...
    def visitAttribute(self, ctx: BKOOLParser.AttributeContext):
        if ctx.EQ():
            ...
            return (ctx.ID().getText(), None)  ## depend on định nghĩa của anh bí
    ...
    def visitConstructor(self, ctx: BKOOLParser.ConstructorContext):
        return str(MethodDecl(Instance(),
                              Id('"<init>"'),
    ...
    def visitIf_stmt(self, ctx: BKOOLParser.If_stmtContext):
        if ctx.else_stmt():
            if ctx.block_stmt():
                ...
                return If(self.visit(ctx.expr()), self.visit(ctx.stmt()), self.visit(ctx.else_stmt()))
        else:
            if ctx.block_stmt():
                return If(self.visit(ctx.expr()), self.visit(ctx.block_stmt()), None)
            else:
                return If(self.visit(ctx.expr()), self.visit(ctx.stmt()), None)
    ...
    def visitFor_stmt(self, ctx: BKOOLParser.For_stmtContext):
        return For(self.visit(ctx.scalar_variable()),
                   ...
                   self.visit(ctx.stmt()) if ctx.stmt() else self.visit(ctx.block_stmt()))

    def visitBreakStmt(self, ctx: BKOOLParser.Break_stmtContext):
        # breakStmt: BREAK SEMI;
        return Break()

    def visitContinueStmt(self, ctx: BKOOLParser.Continue_stmtContext):
        # continueStmt: CONTINUE SEMI;
        return Continue()

    def visitReturnStmt(self, ctx: BKOOLParser.Return_stmtContext):
    ...
    def visitExp10_call(self, ctx: BKOOLParser.Exp10_callContext):
        return CallExpr(Id(ctx.ID().getText()), Id(ctx.getText()), self.visit(ctx.exp_list()))
