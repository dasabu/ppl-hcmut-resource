...
from AST import *
from main.bkool.codegen.CodeGenError import IllegalOperandException
...
class Emitter:
    ...
    *   @param name the symbol entry of the variable.
    ...
    def emitWRITEVAR(self, name, inType, index, frame):
        ...
        if type(inType) is IntType:
            ...
            type(inType) is cgen.ClassType
            or type(inType) is StringType
            or type(inType) is ArrayType
        ):
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitATTRIBUTE(self, lexeme, in_, isFinal, value, isStatic=False):
        # lexeme: String
        # in_: Type
        # isFinal: Boolean
        # value: String
    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if type(in_) is IntType:
            if op == ">":
                result.append(self.jvm.emitIFICMPLE(str(labelF)))
            elif op == ">=":
                result.append(self.jvm.emitIFICMPLT(str(labelF)))
            elif op == "<":
                result.append(self.jvm.emitIFICMPGE(str(labelF)))
            elif op == "<=":
                result.append(self.jvm.emitIFICMPGT(str(labelF)))
            elif op == "!=":
                result.append(self.jvm.emitIFICMPEQ(str(labelF)))
            elif op == "==":
                result.append(self.jvm.emitIFICMPNE(str(labelF)))

        elif type(in_) is FloatType:
            result.append(self.jvm.emitFCMPL())
            if op == ">":
                result.append(self.jvm.emitIFLE(str(labelF)))
            elif op == ">=":
                result.append(self.jvm.emitIFLT(str(labelF)))
            elif op == "<":
                result.append(self.jvm.emitIFGE(str(labelF)))
            elif op == "<=":
                result.append(self.jvm.emitIFGT(str(labelF)))
            elif op == "!=":
                result.append(self.jvm.emitIFEQ(str(labelF)))
            elif op == "==":
                result.append(self.jvm.emitIFNE(str(labelF)))
    ...
    *   @param label the label where the execution continues if the value on top of stack is true.
    ...
    def emitIFTRUE(self, label, frame):
        ...
        frame.pop()
        return self.jvm.emitIFGT(str(label))
    ...
    *   @param label the label where the execution continues if the value on top of stack is false.
    ...
    def emitIFFALSE(self, label, frame):
        ...
        frame.pop()
        return self.jvm.emitIFLE(str(label))
    ...
    def emitIFICMPGT(self, label, frame):
        ...
        frame.pop()
        return self.jvm.emitIFICMPGT(str(label))
    ...
    def emitIFICMPLT(self, label, frame):
        ...
        frame.pop()
        return self.jvm.emitIFICMPLT(str(label))
